...
const find_shortest_string_1 = __importDefault(
  require("./find-shortest-string"),
);
test("findShortestString finds the shortest string in an array", () => {
  expect(find_shortest_string_1.default(["it", "is", "a", "nice", "day"])).toBe(
    "a",
  );
  expect(find_shortest_string_1.default(["why", "hello", "to", "you"])).toBe(
    "to",
  );
});
